        program fghm 
        implicit none
        integer :: nx,i,j,k,np,Ts,NE,nvib,ntime
        INTEGER :: LWORK,INFO
        real*8 ::dp,dx,xmin,xmax,pi,mass,D,beta,Xe,Kb,Z,P,F
        real*8 ::Z1,Z2,Z3,Z4,sigma,omega,time
        real*8, allocatable :: x(:),E(:),F1(:),F2(:),F3(:),a(:)
        REAL*8, ALLOCATABLE :: WORK(:), gauss(:)
        real*8, allocatable :: H(:,:),S(:,:),C(:,:),T(:,:),rho(:,:)
        ...
        Z=0.0 
        Ts=300
         write(6,*)'inserte el numero de puntos (nx)'
        read(5,*)nx
        ...
        enddo
        write(6,*)'Numero de estados vibracionales',nvib
        ...
        do i=1,nvib
           do j=1,nx
              ...
              a(i)=a(i)+C(j,i)*gauss(j)*dx
           enddo
           write(6,*)a(i)
        enddo
        write(6,*)'suma=',sum(a(:)*a(:))
        ntime=1
